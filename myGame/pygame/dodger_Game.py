import pygame
import random
import sys
import base64
import io

SHEET_B64 = ""

# =========================
# 🎧 AUDIO (추가된 부분)
# =========================
pygame.mixer.init()

LASER_SOUND_PATH = "./sound/laser.wav"   # ⭐ 여기에 상대경로 (예: "./laser.wav")
BGM_PATH = "./sound/background.mp3"      # ⭐ 여기에 상대경로 (예: "./bgm.mp3")
warning_sfx = pygame.mixer.Sound("./sound/warning.wav")
warning_sfx.set_volume(0.4)
BACKGROUND_PATH = "./sprite/background.png"
ENEMY_IMAGE_PATH = "./sprite/star.png"  # ⭐ 원하는 상대경로

laser_sfx = None
bgm = None

try:
    if LASER_SOUND_PATH:
        laser_sfx = pygame.mixer.Sound(LASER_SOUND_PATH)

except:
    pass
# =========================

def load_base64_image(b64_string):
    image_data = base64.b64decode(b64_string)
    image_file = io.BytesIO(image_data)
    return pygame.image.load(image_file).convert_alpha()

pygame.init()

info = pygame.display.Info()
WIDTH, HEIGHT = info.current_w, info.current_h

screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.FULLSCREEN)
FPS = 60

WHITE  = (255, 255, 255)
RED    = (220, 50, 50)
GREEN  = (50, 200, 100)
GRAY   = (40, 40, 40)
YELLOW = (240, 200, 0)
BLUE   = (50, 120, 220)

screen = pygame.display.set_mode((WIDTH, HEIGHT))
clock = pygame.time.Clock()
font = pygame.font.SysFont(None, 48)
font_big = pygame.font.SysFont(None, 120)
font = pygame.font.SysFont("malgungothic", 28)  # ⭐ 윈도우 기본 한글폰트

PLAYER_W, PLAYER_H = 80, 60
ENEMY_W, ENEMY_H = 100, 100
ITEM_SIZE = 25

LEVELS = [
    {"min_speed": 5, "max_speed": 8,  "spawn": 40, "label": "Lv.1", "laser_chance": 0.005, "spike_chance": 0.01},
    {"min_speed": 8, "max_speed": 12,  "spawn": 25, "label": "Lv.2", "laser_chance": 0.01, "spike_chance": 0.02},
    {"min_speed": 12, "max_speed": 15 , "spawn": 15, "label": "Lv.3", "laser_chance": 0.015, "spike_chance": 0.03},
]

def spawn_enemy(level):
    x = random.randint(0, WIDTH - ENEMY_W)
    speed = random.randint(level["min_speed"], level["max_speed"])
    return pygame.Rect(x, -ENEMY_H, ENEMY_W, ENEMY_H), speed

def spawn_item():
    x = random.randint(0, WIDTH - ITEM_SIZE)
    speed = random.randint(3, 6)
    return pygame.Rect(x, -ITEM_SIZE, ITEM_SIZE, ITEM_SIZE), speed

def draw_hud(score, level_cfg, lives):
    screen.blit(font.render(f"Score: {score}", True, WHITE), (10, 10))
    screen.blit(font.render(f"{level_cfg['label']}", True, YELLOW), (10, 40))
    screen.blit(font.render(f"Lives: {'♥ ' * lives}", True, RED), (WIDTH - 180, 10))

def game_over(score):
    screen.fill(GRAY)
    screen.blit(font.render(f"Score: {score}", True, WHITE), (320, 320))
    screen.blit(font.render("R: Restart  Q: Quit", True, WHITE), (260, 380))
    pygame.display.flip()
    pygame.mixer.music.stop()  # ⭐ 이거 추가
    screen.fill(GRAY)
    
def game_clear(score):
    screen.fill((0, 0, 0))

    clear_text = font_big.render("CLEAR!", True, (0, 255, 100))
    score_text = font.render(f"Final Score: {score}", True, WHITE)
    info_text = font.render("R: Restart  Q: Quit", True, WHITE)

    clear_rect = clear_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 80))
    score_rect = score_text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
    info_rect = info_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 60))

    screen.blit(clear_text, clear_rect)
    screen.blit(score_text, score_rect)
    screen.blit(info_text, info_rect)

    pygame.display.flip()

    # 텍스트 생성
    gameover_text = font_big.render("GAME OVER", True, RED)
    score_text = font.render(f"Score: {score}", True, WHITE)
    info_text = font.render("R: Restart  Q: Quit", True, WHITE)

    # 중앙 좌표 계산
    gameover_rect = gameover_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 80))
    score_rect = score_text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
    info_rect = info_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 60))

    # 출력
    screen.blit(gameover_text, gameover_rect)
    screen.blit(score_text, score_rect)
    screen.blit(info_text, info_rect)

    pygame.display.flip()

    while True:
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                pygame.quit(); sys.exit()
                
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_ESCAPE:
                    pygame.quit()
                    sys.exit()
                    
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_r:
                    return "restart"
                if e.key == pygame.K_q:
                    pygame.quit(); sys.exit()

def show_start_screen(screen, clock):
    font_big = pygame.font.SysFont(None, 80)
    font_small = pygame.font.SysFont(None, 40)

    while True:
        screen.fill((0, 0, 0))

        title = font_big.render("GameStart", True, (255, 255, 255))
        start = font_small.render("Press SPACE to Start", True, (200, 200, 200))

        title_rect = title.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 100))
        start_rect = start.get_rect(center=(WIDTH // 2, HEIGHT // 2))

        screen.blit(title, title_rect)
        screen.blit(start, start_rect)

        pygame.display.flip()

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()

            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    return  # 게임 시작

        clock.tick(60)

def main():
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT), pygame.FULLSCREEN) 
    clock = pygame.time.Clock()

    show_start_screen(screen, clock)  # ⭐ 여기 추가

    game_cleared = False
    
    enemy_img = None
    try:
        enemy_img = pygame.image.load(ENEMY_IMAGE_PATH).convert_alpha()
        enemy_img = pygame.transform.scale(enemy_img, (ENEMY_W, ENEMY_H))
    except:
        print("적 이미지 로드 실패")
    
    background = None
    try:
        # 이미지를 불러와서 게임 화면 크기로 변환
        loaded_bg = pygame.image.load(BACKGROUND_PATH).convert()
        background = pygame.transform.scale(loaded_bg, (WIDTH, HEIGHT))
        
    except:
        # 파일이 없어도 에러로 멈추지 않게 예외 처리
        print("배경 이미지 로드 실패")
    
    try:
        if BGM_PATH:
            pygame.mixer.music.load(BGM_PATH)
            pygame.mixer.music.play(-1)
            pygame.mixer.music.set_volume(0.3)
    except:
        pass
    
    player = pygame.Rect(WIDTH//2, HEIGHT-100, PLAYER_W, PLAYER_H)
    enemies = []
    items = []
    lasers = []
    spikes = []
    beams = []  # [x_pos, timer] - 빔의 위치와 지속시간을 저장
    final_beam_spawned = False
    final_beam = None
    
    chaos_wait_timer = 0    # 99점 달성 후 2초 대기용
    chaos_timer = 0         # 생성 간격 조절용
    chaos_step = 0          # 현재 단계(0~3)
    chaos_phase = "wait"    # 상태 관리 (wait -> inward -> outward -> final -> end)
    
    # 화면 8등분 x좌표 리스트 (적의 너비 절반을 빼서 중앙 정렬)
    lanes = [i * (WIDTH // 8) + (WIDTH // 16) - (ENEMY_W // 2) for i in range(8)]
    
    # 낙하 순서 (양 끝에서 안으로 / 안에서 밖으로)
    inward_seq = [(0, 7), (1, 6), (2, 5), (3, 4)]
    outward_seq = [(3, 4), (2, 5), (1, 6), (0, 7)]
    

    shake_timer = 0
    shake_intensity = 0
    score = 0
    lives = 3
    spawn_timer = 0
    item_timer = 0

    level_idx = 0
    level = LEVELS[level_idx]
    
    if score >= 99:
        enemies.clear()
        items.clear()
        lasers.clear()
        spikes.clear()

    sheet = load_base64_image(SHEET_B64)
    player_frames = []
    for i in range(3):
        frame = pygame.Surface((32, 32), pygame.SRCALPHA)
        frame.blit(sheet, (0, 0), (i * 32, 0, 32, 32))
        player_frames.append(frame)

    frame_index = 0
    frame_timer = 0
    invincible = 0
    direction = 1  # 1: 오른쪽, -1: 왼쪽
    

    while True:
        clock.tick(FPS)
        
        # 🎨 화면 초기화 (이미지가 있으면 배경을, 없으면 검정색을 칠함)
        if background:
            screen.blit(background, (0, 0))
        else:
            screen.fill((0, 0, 0))
            
            
        for b_x, b_timer in beams:
            # 1. 빔의 투명도 조절
            alpha = int((b_timer / 15) * 255)
            
            # 2. 빔의 폭 설정 (40 -> 100으로 변경)
            beam_width = 100 
            beam_surface = pygame.Surface((beam_width, HEIGHT), pygame.SRCALPHA)
            
            # 3. 빔 색상 및 투명도 채우기
            beam_surface.fill((255, 255, 255, alpha))
            
            # 4. 화면에 그리기 (중앙 정렬을 위해 폭의 절반인 50을 뺌)
            screen.blit(beam_surface, (b_x - (beam_width // 2), 0))
            
            # 5. 중앙의 강렬한 선도 조금 더 두껍게 (5 -> 10)
            pygame.draw.line(screen, (255, 255, 255), (b_x, 0), (b_x, HEIGHT), 10)
            

        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
                
            if e.type == pygame.KEYDOWN:
                if e.key == pygame.K_p:
                    score = 90

        keys = pygame.key.get_pressed()
        if keys[pygame.K_LEFT] and player.left > 0:
            player.x -= 5
            direction = -1
        if keys[pygame.K_RIGHT] and player.right < WIDTH:
            player.x += 5
            direction = 1

        frame_timer += 1
        if frame_timer >= 10:
            frame_timer = 0
            frame_index = (frame_index + 1) % len(player_frames)

        spawn_timer += 1
        if score < 99:
            spawn_timer += 1
            if score < 99 and spawn_timer >= level["spawn"]:
                spawn_timer = 0
                rect, speed = spawn_enemy(level)
                enemies.append([rect, speed, 0, random.randint(1, 3)])
        else:
            # 점수가 99점 이상일 때 실행되는 제빌 패턴 (Final Chaos)
            if chaos_phase == "wait":
                chaos_wait_timer += 1
                if chaos_wait_timer >= 120: # 2초 대기 (60FPS * 2)
                    chaos_phase = "inward"
                    chaos_timer = 0

            elif chaos_phase == "inward":
                chaos_timer += 1
                if chaos_timer % 15 == 0: # 0.25초 간격으로 낫(적) 낙하
                    idx1, idx2 = inward_seq[chaos_step]
                    # enemies.append([Rect, speed, angle, rot_speed])
                    enemies.append([pygame.Rect(lanes[idx1], -ENEMY_H, ENEMY_W, ENEMY_H), 15, 0, 10])
                    enemies.append([pygame.Rect(lanes[idx2], -ENEMY_H, ENEMY_W, ENEMY_H), 15, 0, 10])
                    chaos_step += 1
                    if chaos_step >= len(inward_seq):
                        chaos_step = 0
                        chaos_phase = "outward"

            elif chaos_phase == "outward":
                chaos_timer += 1
                if chaos_timer % 15 == 0:
                    idx1, idx2 = outward_seq[chaos_step]
                    enemies.append([pygame.Rect(lanes[idx1], -ENEMY_H, ENEMY_W, ENEMY_H), 15, 0, 10])
                    enemies.append([pygame.Rect(lanes[idx2], -ENEMY_H, ENEMY_W, ENEMY_H), 15, 0, 10])
                    chaos_step += 1
                    if chaos_step >= len(outward_seq):
                        chaos_phase = "final"
                        chaos_timer = 0
                        
                    if chaos_step >= len(outward_seq):
                        chaos_step = 0
                        chaos_phase = "end"
                        chaos_timer = 0

                        # ⭐ end 진입 보상 1회
                        score += 1
                    
        new_enemies = []
        for rect, speed, angle, rot_speed in enemies:
            rect.y += speed
            if chaos_phase == "end" and not final_beam_spawned:
                if rect.height >= ENEMY_H * 5 and rect.top > HEIGHT:
                    final_beam_spawned = True
                    final_beam = pygame.Rect(0, 0, WIDTH, HEIGHT)

                    if laser_sfx:
                        laser_sfx.play()
                        
            if rect.top > HEIGHT:
                # 마지막 패턴 중에만 빔 생성
                if score >= 99 and chaos_phase not in ["wait"]:
                    beams.append([rect.centerx, 15]) 
                if score < 99: score += 1
            else:
                new_enemies.append([rect, speed, angle + rot_speed, rot_speed])
        enemies = new_enemies

        if score >= 100 and chaos_phase == "end":
            result = game_clear(score)
            if result == "restart":
                main()
            return

        # 빔 타이머 업데이트
        beams = [[x, t - 1] for x, t in beams if t - 1 > 0]

        if score < 99:
            item_timer += 1
            if item_timer >= 120:
                item_timer = 0
                if random.random() < 0.3:
                    items.append(list(spawn_item()))

        # ⭐ 레이저 생성 (SFX 추가됨)
        if score < 99 and random.random() < level["laser_chance"]:
            delay = random.randint(30, 60)
            duration = 18
            lasers.append([player.centerx, 0, delay, duration, False, 0, False])
            # 마지막 False = warning_played
            
        # ⭐ 가시 생성
        if score < 99 and random.random() < level["spike_chance"]:
            x = random.randint(0, WIDTH - 40)
            delay = random.randint(40, 80)
            duration = 20
            spikes.append([x, delay, duration, 0])  
            # x, delay, duration, timer

        new_enemies = []
        for rect, speed, angle, rot_speed in enemies:
            rect.y += speed
            angle += rot_speed  # ⭐ 회전 추가

            if rect.top < HEIGHT:
                new_enemies.append([rect, speed, angle, rot_speed])
            else:
                if score < 99: 
                    score += 1
        enemies = new_enemies
        
        # === [추가] 빔 피격 판정 (넓이에 따라 자동 조절) ===
        beam_width = 100  # 여기서 넓이를 조절하면 판정도 자동으로 바뀝니다.
        
        for b_x, b_timer in beams:
            # 빔의 현재 위치와 넓이를 바탕으로 충돌 박스 생성
            # 그리기 로직과 동일하게 (b_x - 절반) 위치부터 시작
            beam_rect = pygame.Rect(b_x - (beam_width // 2), 0, beam_width, HEIGHT)
            
            # 플레이어와 충돌했는지 검사
            if player.colliderect(beam_rect):
                # 빔이 막 생성되어 활발한 상태일 때만 데미지 (b_timer가 클 때)
                if b_timer > 10:
                    lives -= 1          # 목숨 1개 감소
                    invincible = 60     # 1초 동안 무적 (60프레임)
                    shake_timer = 20    # 화면 흔들림 효과
                    shake_intensity = 10
                    invincible = True
        
        if final_beam_spawned and final_beam:
            beam_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            beam_surface.fill((255, 255, 255, 180))
            screen.blit(beam_surface, (0, 0))
            
        if score >= 99 and chaos_phase == "end" and final_beam_spawned and not game_cleared:
            game_cleared = True

            score += 1  # 보상 점수

            result = game_clear(score)

            if result == "restart":
                main()
                return
        
        
        new_items = []
        for rect, speed in items:
            rect.y += speed
            if rect.top < HEIGHT:
                new_items.append([rect, speed])
        items = new_items

        new_lasers = []
        hitbox = player.inflate(-20, -10)
        for x, timer, delay, duration, locked, state, warning_played in lasers:
            timer += 1

            if state == 0:
                if not warning_played:
                    warning_sfx.play()
                    warning_played = True  # ⭐ 한 번만 재생
                    
                if delay - timer <= 12:
                    locked = True
                if not locked:
                    x = player.centerx
                if timer >= delay:
                    state = 1
                    timer = 0

            elif state == 1:
                if timer >= 3:
                    state = 2
                    timer = 0
                    
                    if laser_sfx:
                        laser_sfx.play()

            elif state == 2:
                duration -= 1
                beam_rect = pygame.Rect(x - 12, 0, 24, HEIGHT)

                if invincible == 0 and hitbox.colliderect(beam_rect):
                    lives -= 1
                    invincible = 90
                    shake_timer = 15
                    shake_intensity = 10

                    if lives <= 0:
                        result = game_over(score)
                        if result == "restart":
                            main()   # 완전히 새 게임 시작
                        return       

                if duration <= 0:
                    continue

            new_lasers.append([x, timer, delay, duration, locked, state, warning_played])

        lasers = new_lasers
        
        # 🔥 충돌 판정용 Rect (플레이어보다 작게)
        
        new_spikes = []
        for x, delay, duration, timer in spikes:
            timer += 1
            
            spike_rect = None  # ⭐ 항상 먼저 초기화

            # 상태 0: ! 표시
            if timer < delay:
                pass

            # 상태 1: 가시 활성
            elif timer < delay + duration:
                spike_rect = pygame.Rect(x + 10, HEIGHT - 60, 20, 40)
        
            if spike_rect and invincible == 0 and hitbox.colliderect(spike_rect):
                lives -= 1
                invincible = 90
                shake_timer = 15
                shake_intensity = 10

                if lives <= 0:
                    result = game_over(score)
                    if result == "restart":
                        main()   # 완전히 새 게임 시작
                    return       
                
            if timer < delay + duration:
                new_spikes.append([x, delay, duration, timer])

            spikes = new_spikes

        if invincible > 0:
            invincible -= 1
        else:
            for rect, _, _, _ in enemies:
                if hitbox.colliderect(rect):
                    lives -= 1
                    invincible = 90
                    shake_timer = 15
                    shake_intensity = 10
                    enemies.clear()

                    if lives <= 0:
                        result = game_over(score)

                        if lives <= 0:
                            result = game_over(score)
                            if result == "restart":
                                main()   # 완전히 새 게임 시작
                            return       

        new_items = []
        for rect, speed in items:
            enemy_hitbox = rect  # AABB 그대로
            
            if hitbox.colliderect(rect):
                if lives < 3:   # ⭐ 3 미만일 때만 회복
                    lives += 1
            else:
                new_items.append([rect, speed])
        items = new_items

        level_idx = min(score // 25, len(LEVELS) - 1)
        level = LEVELS[level_idx]

        
        
        offset_x = 0
        offset_y = 0

        if shake_timer > 0:
            shake_timer -= 1
            offset_x = random.randint(-shake_intensity, shake_intensity)
            offset_y = random.randint(-shake_intensity, shake_intensity)

        if invincible == 0 or (invincible // 10) % 2 == 0:
            img = pygame.transform.scale(player_frames[frame_index], (PLAYER_W, PLAYER_H))
            if direction == -1:
                img = pygame.transform.flip(img, True, False)
            
            screen.blit(img, (player.x + offset_x, player.y + offset_y))

        for rect, _, angle, _ in enemies:
            if enemy_img:
                rotated = pygame.transform.rotozoom(enemy_img, angle, 1)
                new_rect = rotated.get_rect(center=rect.center)
                screen.blit(rotated, (new_rect.x + offset_x, new_rect.y + offset_y))
            else:
                pygame.draw.rect(screen, RED, rect.move(offset_x, offset_y))

        for rect, _ in items:
            pygame.draw.rect(screen, GREEN, rect.move(offset_x, offset_y))

        for x, timer, delay, duration, locked, state, warning_played in lasers:

            if state == 0:
                progress = timer / delay
                blink_speed = max(2, int(10 - progress * 8))

                if (timer // blink_speed) % 2 == 0:
                    pygame.draw.line(screen, (255, 100, 100), (x + offset_x, 0), (x + offset_x, HEIGHT), 5)

            elif state == 2:
                alpha = int(128 + 127 * abs(pygame.math.Vector2(1, 0).rotate(timer * 20).x))
                beam = pygame.Surface((50, HEIGHT), pygame.SRCALPHA)
                beam.fill((255, 255, 255, alpha))
                screen.blit(beam, (x - 25 + offset_x, offset_y))
                
        for x, delay, duration, timer in spikes:

            # 🔴 경고 (! 표시)
            if timer < delay:
                if (timer // 10) % 2 == 0:
                    text = font.render("!", True, RED)
                    screen.blit(text, (x + 10, HEIGHT - 80))

            # 🔥 가시
            else:
                spike_points = [
                    (x + offset_x, HEIGHT + offset_y),
                    (x + 40 + offset_x, HEIGHT + offset_y),
                    (x + 20 + offset_x, HEIGHT - 60 + offset_y)
                ]
                pygame.draw.polygon(screen, RED, spike_points)

        draw_hud(score, level, lives)

        pygame.display.flip()

main()

